...
class OrderService:
    ...
    def fiat_market_buy(self, product_id: str, fiat_amount: str) -> Order:
        ...
        try:
            ...
            if not order_response["success"]:
                ...
                preview_failure_reason = error_response.get(
                    "preview_failure_reason", "Unknown"
                )
                error_log = (
                    f"Failed to place a market buy order. "
            ...
            self._log_order_result(
                order_response, product_id, fiat_amount, side=OrderSide.BUY
            )
            return order
    ...
    def fiat_market_sell(self, product_id: str, fiat_amount: str) -> Order:
        ...
        base_size = calculate_base_size(
            Decimal(fiat_amount), spot_price, base_increment
        )
        ...
        try:
            ...
            if not order_response["success"]:
                ...
                error_log = (
                    f"Failed to place a market sell order. "
                    f"Reason: {error_message}. "
                    f"Preview failure reason: {preview_failure_reason}"
                )
                logger.error(error_log)
            ...
            self._log_order_result(
                order_response, product_id, str(base_size), side=OrderSide.SELL
            )
            return order
        except Exception as e:
            error_message = str(e)
    ...
    def fiat_limit_buy(
        ...
        return self._place_limit_order(
            product_id, fiat_amount, limit_price, price_multiplier, OrderSide.BUY
        )
    ...
    def fiat_limit_sell(
        ...
        """
        return self._place_limit_order(
            product_id, fiat_amount, limit_price, price_multiplier, OrderSide.SELL
        )
    ...
    def _place_limit_order(
        ...
        """
        logger.info(
            f"Starting limit order placement - Side: {side}, Product: {product_id}"
        )
        ...
        adjusted_price = (
            Decimal(limit_price)
            if limit_price
            else current_price * Decimal(str(price_multiplier))
        ).quantize(quote_increment)

        # Calculate base size
        base_size = calculate_base_size(
            Decimal(fiat_amount), adjusted_price, base_increment
        )

        # Place the order
        order_func = (
            self.rest_client.limit_order_gtc_buy
        ...
        if "success_response" not in order_response:
            error_response = order_response.get("error_response", {})
            self._log_order_result(
                order_response, product_id, amount, adjusted_price, side
            )
            raise Exception(f"Order placement failed: {error_response}")
    ...
    def _log_order_result(
        ...
        if order["success"]:
            spot_price = (
                price if price else self.price_service.get_spot_price(product_id)
            )

            if side == OrderSide.BUY:
                if price:  # Limit order
                    base_amount = (Decimal(amount) / Decimal(price)).quantize(
                        base_increment
                    )
                    log_message = (
                        f"Successfully placed a limit buy order "
                    ...
                    base_amount = (Decimal(amount) / Decimal(spot_price)).quantize(
                        base_increment
                    )
                    log_message = (
                        f"Successfully placed a market buy order "
                        f"for {Decimal(amount).quantize(quote_increment)} {quote_currency} of {base_currency} "
                        f"(~{base_amount} {base_currency}) at {Decimal(spot_price).quantize(quote_increment)} {quote_currency}"
                    )
            else:  # SELL
                if price:  # Limit order
                    fiat_amount = (Decimal(amount) * Decimal(price)).quantize(
                        quote_increment
                    )
                    log_message = (
                        f"Successfully placed a limit sell order "
                    ...
                    fiat_amount = (Decimal(amount) * Decimal(spot_price)).quantize(
                        quote_increment
                    )
                    log_message = (
                        f"Successfully placed a market sell order "
                        f"for {fiat_amount} {quote_currency} of {base_currency} "
            ...
            preview_failure_reason = order.get("error_response", {}).get(
                "preview_failure_reason", "Unknown"
            )
            order_type = "limit" if price else "market"
            logger.error(
                f"Failed to place a {order_type} {side_str} order. "
